...
CGFloat const kASBlurDefaultDelay = 0.125f;

/*
 模糊度
 */
CGFloat const kASDefaultBlurScale = 0.2f;

CGFloat const kASBlurDefaultDuration = 0.2f;
CGFloat const kASBlurViewMaxAlpha = 1.f;

CGFloat const kASBlurBounceOutDurationScale = 0.8f;

NSString * const kASBlurDidShowNotification = @"com.AlphaStudio.ASBlurredModel.show";
NSString * const kASBlurDidHidewNotification = @"com.AlphaStudio.ASBlurredModel.hide";

typedef void (^ASBlurCompletion)(void);
...
@interface ASBlurredModel ()
@property (assign, readwrite) BOOL isVisible;
@end

#pragma mark - ASBlurredModel

@implementation ASBlurredModel {
    UIViewController *_controller;
    UIView *_contentView;
    ASCloseButton *_dismissButton;
    ASBlurView *_blurView;
    ASBlurCompletion _completion;
}
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        _dismissButton = [[ASCloseButton alloc] init];
...
- (id)initWithViewController:(UIViewController*)viewController title:(NSString*)title message:(NSString*)message {
    UIView *view = [ASBlurredModel generateModalViewWithTitle:title message:message];
...
- (void)updateSubviews {
    ...
    _blurView = [[ASBlurView alloc] initWithCoverView:_controller.view];
    _blurView.alpha = 1.f;
...
- (void)show {
    [self showWithDuration:kASBlurDefaultDuration delay:0 options:kNilOptions completion:NULL];
}
...
- (void)showWithDuration:(CGFloat)duration delay:(NSTimeInterval)delay options:(UIViewAnimationOptions)options completion:(void (^)(void))completion {
    ...
    [self performSelector:@selector(delayedShow) withObject:nil afterDelay:kASBlurDefaultDelay];
}
...
- (void)delayedShow {
    if (! self.isVisible) {
        ...
        [UIView animateWithDuration:self.animationDuration animations:^{
            ...
            if (finished) {
                [[NSNotificationCenter defaultCenter] postNotificationName:kASBlurDidShowNotification object:nil];
                self.isVisible = YES;
                if (_completion) {
                    _completion();
                }
...
*/
- (void)hide {
    [self hideWithDuration:kASBlurDefaultDuration delay:0 options:kNilOptions completion:NULL];
}
        ...
        [UIView animateWithDuration:duration
                         ...
                         completion:^(BOOL finished){
                             if (finished) {
                                 ...
                                 [[NSNotificationCenter defaultCenter] postNotificationName:kASBlurDidHidewNotification object:nil];
                                 self.isVisible = NO;
                                 if (completion) {
                                     completion();
                                 }
...
- (id)initWithCoverView:(UIView *)view {
    if (self = [super initWithFrame:CGRectMake(0, 0, view.bounds.size.width, view.bounds.size.height)]) {
        ...
        self.image = [blur boxblurImageWithBlur:kASDefaultBlurScale];
    }
    return self;
}
